...
func executeSaveNoteRequest(t *testing.T, withPrivilege bool) *http.Response {
	...
	req, err := http.NewRequest("POST", "/notes-api/notes/saveNote", bytes.NewBuffer(noteJsonBytes))
	if err != nil {
		t.Fatalf("ошибка формирования http запроса: %s", err)
	}
...
func executeDownNoteVersionRequest(t *testing.T, withPrivilege bool) *http.Response {
	...
	w := httptest.NewRecorder()
	req, err := http.NewRequest("POST", "/notes-api/notes/downNoteVersion", strings.NewReader(data.Encode()))
	if err != nil {
		t.Fatalf("ошибка формирования http запроса: %s", err)
	}
	req.Header.Add("Content-Length", strconv.Itoa(len(data.Encode())))
...
func executeUpNoteVersionRequest(t *testing.T, withPrivilege bool) *http.Response {
	...
	w := httptest.NewRecorder()
	req, err := http.NewRequest("POST", "/notes-api/notes/upNoteVersion", strings.NewReader(data.Encode()))
	if err != nil {
		t.Fatalf("ошибка формирования http запроса: %s", err)
	}
	req.Header.Add("Content-Length", strconv.Itoa(len(data.Encode())))
...
func TestRestGetUserNotes(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, err := http.NewRequest("POST", "/notes-api/notes/getUserNotes", strings.NewReader(data.Encode()))
	if err != nil {
		t.Fatalf("ошибка формирования http запроса: %s", err)
	}
	req.Header.Add("Content-Length", strconv.Itoa(len(data.Encode())))
